...
RRT::RRT() : rclcpp::Node("rrt_node"), gen((std::random_device())()) {
    ...
    this->declare_parameter("adaptive_lookahead",   false);
    this->declare_parameter("max_lookahead_dist",   5.0);
    this->declare_parameter("obstacle_speed",       0.5);
    ...
    adaptive_lookahead_ = this->get_parameter("adaptive_lookahead").as_bool();
    max_lookahead_dist_ = this->get_parameter("max_lookahead_dist").as_double();
    obstacle_speed_     = this->get_parameter("obstacle_speed").as_double();
    ...
    sample_pub_    = this->create_publisher<visualization_msgs::msg::Marker>("/rrt_samples", 1);
...
void RRT::pose_callback(const nav_msgs::msg::Odometry::ConstSharedPtr pose_msg) {
    ...
    if (pose_tick_++ % 20 == 0) visualize_waypoints();
    ...
    // --- Initialize tree with root at car origin (local frame) ---
    ...
    for (int i = 0; i < max_iterations_; ++i) {
        ...
        if (!use_rrt_star_) {
            ...
            RCLCPP_DEBUG(get_logger(), "Added node %d: (%.2f, %.2f) parent %d",
                         static_cast<int>(tree.size()) - 1, new_node.x, new_node.y, nearest_idx);
        }
        // ── RRT* ───────────────────────────────────────────────────────────
        else {
    ...
    // Always visualize the tree so it never freezes in RViz
    visualize_tree(tree);
    // RCLCPP_INFO(get_logger(), "RRT finished with %d nodes, %d collisions, path_found=%s",
    //             static_cast<int>(tree.size()), collision_cnt, path_found ? "true" : "false");

    if (!path_found) {
        // Keep republishing the last known path so RViz doesn't freeze
        if (!last_path_.empty()) visualize_path(last_path_);

        // Steer toward the goal direction rather than going straight —
        // prevents blindly committing to the wrong fork at junctions
        ...
        // RCLCPP_WARN(get_logger(), "RRT failed to find a path to the goal after %d iterations", max_iterations_);
    }
}
...
// visualize sampled points for debugging (optional)
void RRT::visualize_sample(RRT_Node &sampled_point) {
...
bool RRT::find_goal(double &goal_local_x, double &goal_local_y) {
    ...
    // 2. If we are already in obstacle mode, keep targeting the latched far
    //    goal waypoint until the car is within lookahead_dist_ of it, which
    //    means we have driven past (or nearly to) the end of the obstacle.
    if (adaptive_lookahead_ && goal_in_obstacle_ && obstacle_goal_idx_ >= 0) {
        double dx_map = waypoints_[obstacle_goal_idx_][0] - car_x_;
        double dy_map = waypoints_[obstacle_goal_idx_][1] - car_y_;
        if (std::hypot(dx_map, dy_map) < lookahead_dist_) {
            // Reached the far goal — exit obstacle mode
            goal_in_obstacle_  = false;
            obstacle_goal_idx_ = -1;
        } else {
            // Still approaching the far goal — keep using it
    ...
    // 3. Find goal at normal lookahead distance
    double lx = 0.0, ly = 0.0;
    int    normal_idx = -1;
    if (!scan_for_goal(lookahead_dist_, lx, ly, normal_idx)) {
        // Fallback handled below
        goal_in_obstacle_ = false;
    } else if (!adaptive_lookahead_) {
        // Normal mode: always use the standard lookahead goal
        goal_local_x = lx;
        goal_local_y = ly;
        return true;
    } else {
        // Adaptive mode: check whether that goal lands inside an obstacle
        int gx, gy;
        bool in_obs = world_to_grid(lx, ly, gx, gy) && grid_occupied(gx, gy);

        if (!in_obs) {
            // Goal is clear — normal operation
            goal_local_x = lx;
            goal_local_y = ly;
            return true;
        }

        // Goal is inside an obstacle — latch into obstacle mode.
        // Jump the goal to max_lookahead_dist_ and record that waypoint index
        // so we stay in obstacle mode until the car physically reaches it.
        double far_lx = 0.0, far_ly = 0.0;
        int    far_idx = -1;
        if (scan_for_goal(max_lookahead_dist_, far_lx, far_ly, far_idx)) {
            goal_in_obstacle_  = true;
            obstacle_goal_idx_ = far_idx;
            goal_local_x = far_lx;
            goal_local_y = far_ly;
            return true;
        }

        // max_lookahead not reachable — use normal goal without latching
        goal_local_x = lx;
        goal_local_y = ly;
        return true;
    }
...
void RRT::execute_path(std::vector<RRT_Node> &path) {
    ...
    const double max_steer = 0.8;  // ~23 deg
    steering_angle = std::max(-max_steer, std::min(max_steer, steering_angle));

    // Speed: slow down on sharp turns; further cap when goal is inside an obstacle
    double base_speed = goal_in_obstacle_ ? obstacle_speed_ : max_speed_;
    double speed = base_speed * (1.0 - 0.5 * std::abs(steering_angle) / max_steer);
