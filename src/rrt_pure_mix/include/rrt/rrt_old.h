...
private:
    ...
    double grid_resolution_ = 0.15;  // meters per cell
    ...
    bool   adaptive_lookahead_    = false; // push goal past obstacles if enabled
    double max_lookahead_dist_    = 5.0;   // lookahead used when goal is in obstacle (meters)
    double obstacle_speed_        = 0.5;   // max speed (m/s) while in obstacle-avoidance mode
    bool   goal_in_obstacle_      = false; // latched true until car reaches obstacle_goal_idx_
    int    obstacle_goal_idx_     = -1;    // waypoint index of the far goal set when obstacle detected
    ...
    void visualize_sample(RRT_Node &sampled_point);
